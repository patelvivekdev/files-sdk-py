...
A [`hook`](/features/onaction) can only watch an operation go by. A **plugin** can change it. Plugins are an opt-in, ordered pipeline you pass to the constructor; each one wraps every operation on the instance and can transform the inputs, veto the call, observe the result - or add entirely new methods.
...
Plugins run **inside** the [`onAction`](/features/onaction) / [`onError`](/features/onerror) hooks but **outside** [retries](/features/retries) and [key prefixing](/features/prefixes):
...
- Plugins see **caller-facing keys** - never the internal [prefixed](/features/prefixes) path. A key-rewriting plugin rewrites before prefixing.
...
- **Buffering transforms break streaming.** Encrypt / compress / scan need the whole body in memory, which is incompatible with unknown-length streams and [resumable uploads](/features/resumable) (which re-read the original body). Gate those plugins the way the core already gates streams.
...
- **`wrap` runs outside the timeout.** Per-attempt [timeouts](/features/timeouts) bound the adapter call, not a slow plugin. The caller's `options` (including `signal`) ride on the operation, so a plugin can opt into cancellation itself.
