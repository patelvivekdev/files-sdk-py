...
<Callout>
  ...
  [`encryption()`](/plugins/encryption), and
  [`versioning()`](/plugins/versioning) plugins ship in the box. They form a
  natural pipeline, run in order: snapshot the prior bytes, vet the upload,
  compress what's left, then encrypt it - `plugins: [versioning(), validation(
  {maxSize}), compression(), encryption(key)]`.
</Callout>
