...
The payload mirrors [`onAction`](/features/onaction) - `type`, the caller-facing `key` / `keys` (or `from` / `to` for `copy` and `move`), and `durationMs` - but `error` is always present and typed as a [`FilesError`](/api/errors), not optional.

## Filtering out aborts

A [cancellation](/features/cancellations) and a [timeout](/features/timeouts) both reject, so both reach `onError` - usually as noise you don't want paging anyone. They carry `aborted: true`; branch on it before reporting.
...
The `code` is one of the four normalized [error codes](/api/errors#codes), so you can route by class - swallow `NotFound`, alert on `Provider`, and so on - rather than parsing messages.

## Not fired for bulk partial failures

The array forms don't reject when only some keys fail - they resolve with the bad keys in `result.errors`. So `onError` stays quiet for a partial batch failure; the whole batch still settles through [`onAction`](/features/onaction). Inspect the returned `errors[]` (or `onAction`'s `result`) to catch per-key failures. See [Bulk actions](/features/bulk#retries-and-hooks).
