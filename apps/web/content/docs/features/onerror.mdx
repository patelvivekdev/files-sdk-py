...
The payload mirrors [`onAction`](/features/onaction) - `type`, the caller-facing `key` / `keys` (or `from` / `to` for `copy`), and `durationMs` - but `error` is always present and typed as a [`FilesError`](/api/errors), not optional.
