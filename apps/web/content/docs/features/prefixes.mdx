...
The prefix is an internal detail — it never leaks back to you. Results (`key`, `name`) come back relative, and [hook](/features/onaction) payloads report the `key` / `keys` you passed, never the prefixed path the adapter saw. The same holds in the [bulk](/features/bulk) forms: keys are resolved under the prefix on the way in and stripped on the way out, just as in the single-key forms.
