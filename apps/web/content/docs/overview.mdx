---
title: Overview
description: Files SDK is a unified storage API for 30+ providers - one class, eight methods, a typed escape hatch, and an agent-friendly CLI.
---
...
Nine methods cover the surface area you actually use: `upload`, `download`, `head`, `exists`, `delete`, `deleteMany`, `copy`, `list`, `url`, `signedUploadUrl`. When you need provider-specific power - S3 versioning, lifecycle rules, multipart, ACLs - drop down to the native client via `files.raw`, which stays typed per adapter.
