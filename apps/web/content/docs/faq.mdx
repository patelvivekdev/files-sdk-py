...
Every method throws a single `FilesError` with a normalized `code` - `NotFound`, `Unauthorized`, `Conflict`, or `Provider` - and the original provider error attached as `cause`. See [Errors](/api/errors).
