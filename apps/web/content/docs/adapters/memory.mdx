...
There's no server backing the store, so `url()` returns an opaque, non-fetchable `memory://${key}` URL (it throws `NotFound` for a missing key, the way a real fetch would 404), and `signedUploadUrl()` returns a `memory://` placeholder PUT target. Neither resolves to anything you can actually fetch - they exist so URL-building call sites stay exercisable in tests. For working URLs in dev, use the `fs` adapter with a `urlBaseUrl`, or a real cloud adapter.
