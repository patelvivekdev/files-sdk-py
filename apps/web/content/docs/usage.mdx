---
title: Usage
description: Construct a Files instance with an adapter, then call the same nine methods on it - swap the adapter to switch backends.
---
...
Read-only instances still allow `download`, `head`, `exists`, `list`, `listAll`, and `url`. The write surfaces - `upload`, `delete`, `copy`, `move`, `signedUploadUrl`, and the equivalent `file(key)` helpers - throw [`FilesError`](/api/errors) with `code: "ReadOnly"`. The [`raw`](/features/escape-hatch) escape hatch stays unchanged.
...
See [Bulk actions](/features/bulk) for the per-method result shapes, partial-failure handling, and how `concurrency` and `stopOnError` tune the fan-out.
...
See [Prefixes](/features/prefixes) and the per-operation options ([Timeouts](/features/timeouts), [Retries](/features/retries), [Cancellation](/features/cancellations), [Hooks](#hooks)) for the full behavior.

## Hooks

Pass `hooks` to the constructor to observe operations as they run - one place to wire logging, metrics, error reporting, and retry telemetry without wrapping every call. Each hook is **fire-and-forget**, like the [`onProgress`](/features/onprogress) upload callback: the SDK calls it but never awaits it, and a hook that throws can't fail the operation it observes. Payloads are caller-facing - the public `key` / `keys` you passed, never the internal prefixed path.
...
Three hooks live on the constructor: [`onAction`](/features/onaction) runs when a call settles, [`onError`](/features/onerror) when it rejects, and [`onRetry`](/features/onretry) before each scheduled retry. A fourth callback, [`onProgress`](/features/onprogress), is a per-call upload option rather than a constructor hook, but follows the same fire-and-forget contract.
