...
import { TracingPanel, TRACING_ACTION_FRAMES } from "./tracing-panel";
...
const LINES: Line[] = [
  [
    ...
    ["{ tracing } "],
    ["from ", "kw"],
    ['"files-sdk/tracing"', "str"],
    [";"],
  ],
  [],
  [["const ", "kw"], ["files = "], ["createFiles", "at"], ["({"]],
  [["  "], ["adapter", "at"], [": "], ["s3", "at"], ["(),"]],
  [["  "], ["plugins", "at"], [": ["], ["tracing", "at"], ["()],"]],
  [["});"]],
  [],
  [["// one OpenTelemetry span per call", "cm"]],
  [
    ["await ", "kw"],
    ["files"],
    ["."],
...
export const TRACING_SCENE_DURATION =
  ACTION_START + TRACING_ACTION_FRAMES + DWELL + EXIT;

export const TracingScene: React.FC = () => {
  ...
  const exitOpacity = interpolate(
    frame,
    [TRACING_SCENE_DURATION - EXIT, TRACING_SCENE_DURATION - 2],
    [1, 0],
  ...
  return (
    ...
    >
      <SceneTitle eyebrow="Plugin · tracing" title="Trace every call." />
      <div style={{ alignItems: "center", display: "flex", gap: 56 }}>
        <CodeWindow
          ...
          width={720}
        />
        <TracingPanel frame={frame - ACTION_START} />
      </div>
    </AbsoluteFill>
  );
};
