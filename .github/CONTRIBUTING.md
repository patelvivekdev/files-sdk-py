...
Thanks for your interest in contributing! Files SDK is a unified storage SDK for object/blob backends (AWS S3, Cloudflare R2, Vercel Blob, MinIO). Bug reports, new adapters, docs improvements, and discussion are all welcome.
...
- `packages/files-sdk` — the SDK itself
  ...
  - `src/s3/`, `src/r2/`, `src/vercel-blob/`, `src/minio/` — adapter implementations, each exposed as its own subpath export (`files-sdk/s3`, etc.)
...
- The adapter you're using (S3, R2, Vercel Blob, MinIO)
